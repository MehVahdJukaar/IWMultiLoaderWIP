...
import com.ordana.immersive_weathering.registry_delete.blocks.LeafPileBlock;
...
public class FallingLeafLayerEntity extends ImprovedFallingBlockEntity {
    ...
    public FallingLeafLayerEntity(EntityType<FallingLeafLayerEntity> type, Level level) {
        super(type, level);
    }

    public FallingLeafLayerEntity(Level level) {
        super(ModEntities.FALLING_LEAF_LAYER, level);
    }

    public FallingLeafLayerEntity(Level level, BlockPos pos, BlockState blockState) {
        super(ModEntities.FALLING_LEAF_LAYER, level, pos, blockState, false);
    }

    public static FallingLeafLayerEntity fall(Level level, BlockPos pos, BlockState state) {
                    ...
                    if (!onState.is(Blocks.MOVING_PISTON)) {
                        ...
                        if (canBeReplaced && canSurvive) {
                            ...
                            if (onState.is(blockState.getBlock())) {
                                int layers = blockState.getValue(LeafPileBlock.LAYERS);
                                int toLayers = onState.getValue(LeafPileBlock.LAYERS);
                                int total = layers + toLayers;
                                int target = Mth.clamp(total, 1, 8);
                                remaining = total - target;
                                blockState = blockState.setValue(LeafPileBlock.LAYERS, target);
                            }
                            ...
                            if (this.level.setBlock(pos, blockState, 3)) {
                                ...
                                if (remaining != 0) {
                                    ...
                                    blockState = blockState.setValue(LeafPileBlock.LAYERS, remaining);
    ...
    public static boolean isFree(BlockState pState) {
        ...
        return pState.isAir() || pState.is(BlockTags.FIRE) || material.isLiquid() || (material.isReplaceable() && !(pState.getBlock() instanceof LeafPileBlock));
    }
